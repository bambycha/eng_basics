...
describe Fibs do
  ...
  describe '#is_known_fib?' do
    context 'when passed a fibonacci number that it has not stored' do
      it 'returns false' do
        subject.generate(7)
        expect( subject.is_known_fib?(13) ).to be_falsey
      end
    end
    ...
    context 'when passed a number that has been stored' do
      it 'returns true' do
        subject.generate(7)
        expect( subject.is_known_fib?(8) ).to be_truthy
      end
    end
  end
  ...
  describe '#all_fibs?' do
    context 'when passed an array of numbers that are all members of the the fibonacci sequence' do
      it 'returns true' do
        subject.generate(9)
        expect( subject.all_fibs?([2, 3, 5, 8]) ).to be_truthy
      end
    end
    ...
    context 'when passed an array of numbers that where at least one is not a member of the sequence' do
      it 'returns false' do
        subject.generate(9)
        expect( subject.all_fibs?([21, 34, 55, 89]) ).to be_falsey
      end
    end
  end
